function mesh = mesh_1d_deg(a,b,Ne,p)
%MESH_1D_DEG  Build a 1D uniform mesh for degree-p FEM on [a,b].
%
% Inputs
...
%   mesh.elem = (Ne x (p+1)) connectivity, consecutive global nodes per element
%   mesh.bound.dirichlet = []  (fill in the main script)
%   mesh.bound.neumann   = []  (fill in the main script)
%
% Example
%   mesh = mesh_1d_deg(0,1,10,3);
%   % later in main:
%   mesh.bound.dirichlet = [1; size(mesh.x,1)];
%   mesh.bound.gD = 0;
%
% Notes
...
assert(isscalar(p) && p>=1 && p==floor(p), 'p must be integer >=1');
